...
root_path = './'
raw_data_file = osp.join(root_path, 'raw_data', 'raw_skes_data.pkl')  #原始数据路径
save_path = osp.join(root_path, 'denoised_data')   #去除无效数据后的储存路径

if not osp.exists(save_path):
    os.mkdir(save_path)

rgb_ske_path = osp.join(save_path, 'rgb+ske')    #在储存路径下新建两个文件夹
...
missing_count = 0
noise_len_thres = 11        #视频最少需要有11帧
noise_spr_thres1 = 0.8      #有效帧的横纵比阈值
...
def denoising_by_length(ske_name, bodies_data):   #过滤掉过短的视频数据
    """
...
def get_valid_frames_by_spread(points):   #如果骨架横向的最大宽度小于纵向最大宽度的0.8倍，就说明这一帧是有效的 why????
    """
...
def denoising_by_spread(ske_name, bodies_data):
    ...
    for (bodyID, body_data) in new_bodies_data.items():
        if len(bodies_data) == 1:
            break
        valid_frames = get_valid_frames_by_spread(body_data['joints'].reshape(-1, 25, 3)) #获取横纵比合理的帧
        ...
        if ratio >= noise_spr_thres2:   # 0.69754 如果舍弃的帧过多，这个视频就直接舍弃了
                                              #（避免的是多人连在一起识别成一个人的情况）
            del bodies_data[bodyID]
        ...
        else:  # Update motion   如果没有那么多错误的帧，就将错误的帧删除就行了
...
def denoising_bodies_data(bodies_data):    #将上面三种去噪方法放在一起
    """
...
def get_one_actor_points(body_data, num_frames):
    ...
    joints[start:end + 1] = body_data['joints'].reshape(-1, 75)   #将骨骼点数据变成一个数组
    colors[start:end + 1, 0] = body_data['colors']

    return joints, colors


def remove_missing_frames(ske_name, joints, colors):   #如果横纵坐标有0值代表丢失，就舍弃这个视频
    """
    ...
    if num_bodies == 2:  # DEBUG
        missing_indices_1 = np.where(joints[:, :75].sum(axis=1) == 0)[0]  #横纵坐标为0的点的数量
...
def get_raw_denoised_data(): #最终输出的是长度为150（25*3*2），宽度是帧数的向量，如果只有一个人，最后75行都置零
    """
