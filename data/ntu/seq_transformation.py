...
camera_file = osp.join(stat_path, 'camera.txt')      #camera.txt储存的是每一个数据是用哪个相机照的
...
def seq_translation(skes_joints):   #更新骨架序列数据
...
def frame_translation(skes_joints, skes_name, frames_cnt):  #让每一帧中的骨架尽量居中
    ...
    for idx, ske_joints in enumerate(skes_joints):
        num_frames = ske_joints.shape[0]
        # Calculate the distance between spine base (joint-1) and spine (joint-21)   计算脊柱的长度
        j1 = ske_joints[:, 0:3]
...
def one_hot_vector(labels):   #将标号变成one hot编码
    num_skes = len(labels)
...
def split_train_val(train_indices, method='sklearn', ratio=0.05):   #分离数据获得验证集
    """
...
def split_dataset(skes_joints, label, performer, camera, evaluation, save_path):
    ...
    train_indices, val_indices = split_train_val(train_indices, m)  #从训练集中分出验证集

    # Save labels and num_frames for each sequence of each data set
    train_labels = label[train_indices]  #得到他们的标签
...
def split_class(skes_joints, label, performer, camera, evaluation, class_indexs,save_path):    #利用特定类别的索引将不同类的数据分开  class_indexs是一个类别数组
    train_indices, test_indices = get_indices(performer, camera, evaluation)  #这里面test_indics需要和class_ind取交集
    all_indices=[]   #储存着所有类别的索引序列
    all_labels=[]
    for class_index in class_indexs:
        indices=get_index_classify(label,class_index)
        indices = [i for i in indices if i in test_indices]
        all_indices.append(indices)
        labels=label[indices]
        all_labels.append(labels)
    
        
    h5file = h5py.File(osp.join(save_path, 'NTU_class.h5' ), 'w')
    i=1
    for indices, labels in zip(all_indices, all_labels):
        h5file.create_dataset('x_%d'%(i), data=skes_joints[indices])
        one_hot_labels = one_hot_vector(labels)
        h5file.create_dataset('y_%d'%(i), data=one_hot_labels)
        i=i+1
        
    h5file.close()     #最后将所有的类别数据分开放到了h5py文件NTU_class中
        

def get_indices(performer, camera, evaluation='CS'):     #确定哪些是训练集、评估集、测试集
    ...
    if evaluation == 'CS':  # Cross Subject (Subject IDs)
        ...
        for idx in test_ids:
            temp = np.where(performer == idx)[0]  # 0-based index    np.where用来判断数组中每一个元素是否符合某种性质
            test_indices = np.hstack((test_indices, temp)).astype(np.int)   #temp储存的就是对应这些subject的总视频序号
...
def get_index_classify(label,classify):   #得到对应类别的索引
    indices=np.empty(0)
    temp = np.where(label == classify)[0]
...
if __name__ == '__main__':
    ...
    evaluations = ['CS', 'CV']    #所谓CV（cross view）就是用不同的摄像头作为划分验证集和训练集 
                                  #CS(cross subjects)是用不同的人的数据
        
    
    for evaluation in evaluations:
        split_dataset(skes_joints, label, performer, camera, evaluation, save_path)
    split_class(skes_joints, label, performer, camera, 'CS', range(1,61), save_path)
