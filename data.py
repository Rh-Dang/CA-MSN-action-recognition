...
class NTUDataLoaders(object):
    def __init__(self, dataset ='NTU', case = 0, aug = 1, seg = 30):
        ...
        self.class_sets=[]
        for classx, classy in zip(self.class_X,self.class_Y):
            class_set = NTUDataset(classx, classy)
            self.class_sets.append(class_set)
    ...
    def get_classtest_loader(self, batch_size, num_workers,class_num):
        return DataLoader(self.class_sets[class_num-1], batch_size=batch_size,
                          shuffle=False, num_workers=num_workers,
                          collate_fn=self.collate_fn_fix_test, pin_memory=True, drop_last=True)
    ...
    def create_datasets(self):  #先把数据拎出来
        if self.dataset == 'NTU':
            if self.case ==0:
                self.metric = 'CS'
            elif self.case == 1:
                self.metric = 'CV'
            path = osp.join('./data/ntu', 'NTU_' + self.metric + '.h5')   ##这个地方改变数据集的路径
        ...
        if self.dataset == 'NTU120':
            if self.case ==0:
                ...
                self.metric = 'C-Setup'
            path = osp.join('./data/ntu_120', 'NTU_' + self.metric + '.h5')   ##这个地方改变数据集的路径
        ...
        if self.dataset == 'SYSU':
            if self.case ==0:
                ...
                self.metric = 'SS'
            path = osp.join('./data/SYSU', 'SYSU_' + self.metric + '.h5')   ##这个地方改变数据集的路径
        ...
        path_class=osp.join('./data/ntu','NTU_class.h5')  #用来得到每个class数据
        f_class=h5py.File(path_class,'r')
        for i in range(1,61):
            class_x=f_class['x_%d'%i][:]
            class_y=np.argmax(f_class['y_%d'%i][:], -1)
            self.class_X.append(class_x)
            self.class_Y.append(class_y)
    ...
    def sub_seq(self, seqs, seq , train = 1):   #SYSU就是这里和其他的数据集不一样，剩下的都一样
        group = self.seg
